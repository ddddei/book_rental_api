...
- 머지된 PR: #1 빠른 반납/bookCode 수정, #2 page.tsx 컴포넌트 분리, #3 도서 목록 UX 개선, #4~#5 문서 추가, #6 ops 정리, #7 디자인 토큰 코드화 + 메인 페이지 리뉴얼(B+C 절충 시안), #12 배포 V1 통합 준비(#8~#11 통합), #13 운영자 PIN 리다이렉트 핫픽스, #14 배포 V1 실기기 QA 기록, #15 키보드 단축키 + 모바일 하단 액션 바, #17 데이터 접근 계층 분리(#16 재생성)
...
- [x] vitest 도입 (테스트 40개: book/operatorAccess/deploymentReadiness/api)
- [x] 디자인 후속 (PR #15): 키보드 단축키(`/` 검색, `S` 스캔, `N` 등록), 모바일 하단 고정 액션 바
- [x] 로드맵 PHASE A 완료 (PR #17): 데이터 접근 계층 분리 — 클라이언트 API 호출은 `src/lib/api.ts`만 사용
- [ ] 배포 1차 후속: 1~2주 실사용 피드백 수집 → 백로그 반영 (DEPLOYMENT_PLAN §8-8)
- [ ] 로드맵 PHASE B: Supabase 마이그레이션 (계정/프로젝트 생성 대기 중 → 스키마 생성 → 데이터 이관 → API 교체)
